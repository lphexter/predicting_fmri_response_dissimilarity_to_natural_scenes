    ...
    try:
        ...
        if clip_config.LOAD_EMBEDDINGS_FILE != "":
            ...
            if is_thingsvision:  # process thingsvision embeddings with numpy
                ...
                embeddings = torch.from_numpy(np.load(clip_config.LOAD_EMBEDDINGS_FILE)).to(
                    device
                )  # Move to device (CPU/GPU) if needed
            embeddings = embeddings[:desired_image_number]
        ...
        if embeddings is None or not isinstance(embeddings, type_embeddings):
